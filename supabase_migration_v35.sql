...
BEGIN
  ...
  v_summary := CASE TG_TABLE_NAME
    WHEN 'transactions' THEN INITCAP(COALESCE(v_row.type, 'transaction')) || ' ৳' || COALESCE(v_row.amount::TEXT, '?') || COALESCE(' — ' || NULLIF(v_row.description, ''), '')
    WHEN 'transfers'    THEN 'Transfer ৳' || COALESCE(v_row.amount::TEXT, '?')
    WHEN 'liabilities'  THEN COALESCE(v_row.name, 'Liability') || ' (৳' || COALESCE(v_row.principal::TEXT, '?') || ')'
    WHEN 'accounts'     THEN 'Account ' || COALESCE(v_row.name, '?')
    ELSE TG_TABLE_NAME
  END;
