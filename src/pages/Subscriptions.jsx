...
export default function Subscriptions() {
  ...
  if (loading) return <div className="text-foreground/50 p-6">Loading subscriptions...</div>;
    ...
    <div className="space-y-6 animate-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {subs.map(sub => {
          ...
          return (
            <div key={sub.id} className={`bg-card border rounded-2xl p-5 transition-all ${sub.is_active ? 'border-foreground/10 hover:border-foreground/20' : 'border-foreground/5 opacity-60'}`}>
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="text-foreground font-medium">{sub.title}</h3>
                  <p className="text-foreground/40 text-xs capitalize mt-0.5">
                    {sub.categories?.icon} {sub.categories?.name} · {sub.frequency}
                  </p>
                </div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${sub.is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-foreground/10 text-white/40'}`}>
                  {sub.is_active ? 'ACTIVE' : 'PAUSED'}
                </span>
              </div>
              <div className="flex items-end justify-between">
                <div>
                  <p className="text-xl font-bold text-foreground">{fmt(sub.amount)}<span className="text-xs text-foreground/40 font-normal"> /{sub.frequency.replace('ly', '')}</span></p>
                  {sub.frequency !== 'monthly' && <p className="text-xs text-foreground/40">≈ {fmt(monthly)}/month</p>}
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl font-medium transition-all ${
                    sub.is_active
                      ? 'bg-foreground/5 text-white/60 hover:bg-amber-500/10 hover:text-amber-400'
              ...
              {!sub.is_active && (
                <p className="text-[11px] text-emerald-400/70 mt-3 pt-3 border-t border-foreground/5">
      ...
      {subs.length === 0 && (
        <div className="text-center py-12 border border-foreground/5 rounded-2xl bg-white/[0.02]">
          <Tv className="mx-auto text-foreground/20 mb-4" size={48} />
          <h3 className="text-foreground/60 font-medium">No subscriptions yet</h3>
          <p className="text-foreground/40 text-sm mt-1">
