...
export default function Documents() {
  ...
  if (!docs) return <div className="text-white/50 p-6">Loading documents...</div>;
    ...
    <div className="space-y-6 animate-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">NID, TIN, passport and other important scans in one place.</p>
        </div>
        ...
        <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6">
          ...
          <form onSubmit={handleUpload} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
              ...
              <input type="text" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="e.g. My NID" className="w-full bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-cyan-500/50" />
            </div>
            <div>
              <label className="block text-sm text-white/60 mb-1">Category</label>
              <select value={form.doc_category} onChange={e => setForm({ ...form, doc_category: e.target.value })} className="w-full bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-cyan-500/50">
                {Object.entries(CAT_META).map(([k, v]) => <option key={k} value={k}>{v.icon} {v.label}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm text-white/60 mb-1">Expiry Date (optional)</label>
            ...
            <div>
              ...
              <input required type="file" onChange={e => setForm({ ...form, file: e.target.files?.[0] || null })} className="w-full bg-[#12122a] border border-white/10 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-cyan-500/50" />
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {docs.map(doc => {
          ...
          return (
            <div key={doc.id} className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-all">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full bg-[#12122a] flex items-center justify-center text-xl">{meta.icon}</div>
                  <div>
                    <h3 className="text-white font-medium">{doc.title || doc.file_name}</h3>
                    <p className="text-white/40 text-xs">{meta.label}</p>
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <a href={doc.file_url} target="_blank" rel="noreferrer" className="text-white/40 hover:text-cyan-400 p-1.5 bg-white/5 hover:bg-cyan-500/10 rounded-lg">
                    <ExternalLink size={15} />
                  </a>
                  <button onClick={() => handleDelete(doc)} className="text-white/40 hover:text-red-400 p-1.5 bg-white/5 hover:bg-red-500/10 rounded-lg">
                    <Trash2 size={15} />
                  </button>
                </div>
              </div>
              {doc.expiry_date && (
                <div className="pt-3 border-t border-white/5 text-sm flex items-center justify-between">
                  <span className="text-white/40">Expires</span>
                  <span className={`flex items-center gap-1 ${expired ? 'text-red-400 font-medium' : expiresSoon ? 'text-amber-400' : 'text-white/70'}`}>
      ...
      {docs.length === 0 && !adding && (
        <div className="text-center py-12 border border-white/5 rounded-2xl bg-white/[0.02]">
          ...
          <h3 className="text-white/60 font-medium">No documents yet</h3>
          <p className="text-white/40 text-sm mt-1">Upload NID, TIN, passport or other important scans.</p>
        </div>
      )}
    </div>
  );
