...
export default function FamilyMembers() {
  ...
  if (loading) return <div className="text-foreground/50 p-6">Loading family members...</div>;
    ...
    <div className="space-y-6 animate-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-foreground/40 text-sm mt-1">Manage profiles for shared tracking.</p>
        </div>
        <button
      ...
      {(isAdding || editingMember) && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">{editingMember ? 'Edit Member' : 'New Member'}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Full Name</label>
              <input required type="text" value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50" placeholder="e.g. Jane Doe" />
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Relationship</label>
              <select value={form.relationship} onChange={e => setForm({...form, relationship: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50">
            ...
            <div>
              ...
              <input type="date" value={form.date_of_birth} onChange={e => setForm({...form, date_of_birth: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm text-foreground/60 mb-1">Notes</label>
              <textarea value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50" rows={2} />
            </div>
            <div className="sm:col-span-2 flex justify-end gap-3 mt-2">
              <button type="button" onClick={() => {setIsAdding(false); setEditingMember(null);}} className="px-5 py-2.5 rounded-xl text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-colors">Cancel</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {members.map(member => (
          <div key={member.id} className="bg-card border border-foreground/10 rounded-2xl p-5 hover:border-foreground/20 transition-all group flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white mb-3 shadow-lg shadow-pink-500/20 relative">
              ...
              <div className="absolute top-0 right-0 flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                <button onClick={() => { setEditingMember(member); setForm(member); setIsAdding(false); }} className="text-foreground hover:text-cyan-400 p-1 bg-muted border border-foreground/10 rounded-lg">
                  <Edit2 size={14} />
                </button>
                <button onClick={() => { if (confirm(`Remove "${member.name}"?`)) deleteMember(member.id).catch(err => alert("Cannot delete: " + err.message)); }} className="text-foreground hover:text-red-400 p-1 bg-muted border border-foreground/10 rounded-lg">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
            ...
            {member.date_of_birth && (
              <p className="text-foreground/40 text-xs mt-2">{calculateAge(member.date_of_birth)} years old</p>
            )}
            {memberSpend[member.id] > 0 && (
              <div className="flex items-center gap-1.5 text-foreground/50 text-xs mt-3 bg-foreground/5 rounded-lg px-3 py-1.5">
                <Wallet size={12} />
      ...
      {members.length === 0 && !isAdding && (
        <div className="text-center py-12 border border-foreground/5 rounded-2xl bg-white/[0.02]">
          ...
          <p className="text-foreground/40 text-sm mt-1">Add your spouse or children to track shared finances.</p>
        </div>
      )}
    </div>
  );
