...
export default function Reconcile() {
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      {step === 1 && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-8 text-center space-y-4">
          <ScanSearch className="mx-auto text-foreground/20" size={48} />
          <div>
            <label className="block text-sm text-foreground/60 mb-2">Which account is this statement for?</label>
            <select value={accountId} onChange={e => setAccountId(e.target.value)} className="w-full max-w-sm mx-auto bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50">
              <option value="">Select an account...</option>
              {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
            </select>
          </div>
          <label className={`inline-flex items-center gap-2 px-6 py-3 rounded-xl font-medium cursor-pointer transition-colors ${accountId ? 'bg-cyan-500 hover:bg-cyan-600 text-white' : 'bg-foreground/5 text-white/30 cursor-not-allowed'}`}>
      ...
      {step === 2 && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-4">
          ...
          <p className="text-sm text-foreground/40">{csvRows.length} rows found. Match your CSV's columns to date/description/amount (negative = expense).</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {['date', 'description', 'amount'].map(field => (
              <div key={field}>
                <label className="block text-sm text-foreground/60 mb-1 capitalize">{field}</label>
                <select value={colMap[field]} onChange={e => setColMap({ ...colMap, [field]: e.target.value })} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => setStep(1)} className="px-5 py-2.5 rounded-xl text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-colors">Back</button>
        ...
        <>
          ...
          {diff.missingInApp.length > 0 ? (
            <div className="bg-card border border-foreground/10 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-foreground/10 flex flex-wrap items-center gap-3">
                <h2 className="text-foreground font-semibold">Missing In App — import these?</h2>
                <div className="ml-auto flex gap-3">
                  {missingExpenseCount > 0 && (
                    <select value={expenseCategoryId} onChange={e => setExpenseCategoryId(e.target.value)} className="bg-muted border border-foreground/10 rounded-xl px-3 py-2 text-foreground text-sm focus:outline-none focus:border-cyan-500/50">
                      <option value="">Expense category...</option>
                      {expenseCategories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
                    </select>
                  )}
                  {missingIncomeCount > 0 && (
                    <select value={incomeCategoryId} onChange={e => setIncomeCategoryId(e.target.value)} className="bg-muted border border-foreground/10 rounded-xl px-3 py-2 text-foreground text-sm focus:outline-none focus:border-cyan-500/50">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-foreground/5 border-b border-foreground/10">
                      ...
                      <th className="text-left py-2.5 px-5 text-foreground/60 font-medium">Date</th>
                      <th className="text-left py-2.5 px-5 text-foreground/60 font-medium">Description</th>
                      <th className="text-left py-2.5 px-5 text-foreground/60 font-medium">Type</th>
                      <th className="text-right py-2.5 px-5 text-foreground/60 font-medium">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {diff.missingInApp.map((r, i) => (
                      <tr key={i} className="border-b border-foreground/5 hover:bg-white/[0.02]">
                        ...
                        <td className="py-2 px-5 text-foreground/70">{r.date}</td>
                        <td className="py-2 px-5 text-foreground/60">{r.description}</td>
                        <td className="py-2 px-5"><span className={r.type === 'expense' ? 'text-red-400' : 'text-emerald-400'}>{r.type}</span></td>
                        <td className="py-2 px-5 text-right text-foreground">{fmt(r.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <div className="px-5 py-4 border-t border-foreground/10 flex justify-end gap-3">
                <button onClick={reset} className="px-5 py-2.5 rounded-xl text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-colors">Start Over</button>
            ...
            <div className="text-center py-12 border border-foreground/5 rounded-2xl bg-white/[0.02]">
              ...
              <h3 className="text-foreground/60 font-medium">Everything in the statement is already in your ledger.</h3>
          ...
          {diff.staleInApp.length > 0 && (
            <div className="bg-card border border-foreground/10 rounded-2xl overflow-hidden">
              <h2 className="text-foreground font-semibold px-5 pt-4 pb-2">In App, Not Seen In Statement</h2>
              <p className="text-xs text-foreground/40 px-5 pb-3">These app transactions in the same date range didn't match a statement row — worth a second look.</p>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <tbody>
                    {diff.staleInApp.map(t => (
                      <tr key={t.id} className="border-t border-foreground/5">
                        <td className="py-2 px-5 text-foreground/70">{new Date(t.date).toLocaleDateString()}</td>
                        <td className="py-2 px-5 text-foreground/60">{t.description || t.categories?.name || '—'}</td>
                        <td className="py-2 px-5 text-right text-foreground">{fmt(t.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {result && (
        <div className="text-center py-12 border border-emerald-500/20 bg-emerald-500/5 rounded-2xl">
          ...
          <h3 className="text-foreground font-semibold text-lg">Imported {result.count} transactions</h3>
          <button onClick={reset} className="mt-4 text-sm text-cyan-400 hover:underline">Reconcile another statement</button>
        </div>
      )}
    </div>
