...
export default function Transfers() {
  ...
  if (loadingTransfers) return <div className="text-foreground/50 p-6">Loading transfers...</div>;
    ...
    <div className="space-y-6 animate-in">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-foreground/40 text-sm mt-1">Move funds between your accounts.</p>
        </div>
        <button
      ...
      {accounts.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {accounts.map(acc => (
            <div key={acc.id} className="bg-foreground/5 border border-foreground/10 rounded-xl p-3">
              <p className="text-foreground/40 text-xs truncate">{acc.name}</p>
              <p className="text-foreground font-semibold mt-1">{acc.currency}{Number(acc.current_balance).toLocaleString()}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {isAdding && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">Record a Transfer</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-foreground/60 mb-1">From Account</label>
              <select required value={form.from_account_id} onChange={e => setForm({...form, from_account_id: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50">
                <option value="">Select Account</option>
                {accounts.map(a => <option key={a.id} value={a.id}>{a.name} ({a.currency}{a.current_balance})</option>)}
              </select>
              {fromAccount && (
                <p className="text-xs text-foreground/40 mt-1">Available: {fromAccount.currency}{Number(fromAccount.current_balance).toLocaleString()}</p>
              )}
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">To Account</label>
              <select required value={form.to_account_id} onChange={e => setForm({...form, to_account_id: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50">
                <option value="">Select Account</option>
            ...
            <div>
              ...
              <input required type="number" step="0.01" min="0.01" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
            </div>
            <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-foreground/5 border-b border-foreground/10">
                ...
                <th className="text-left py-3 px-4 text-foreground/60 font-medium">Transfer</th>
            ...
            <tbody>
              {transfers.length === 0 ? (
                <tr>
                  <td colSpan="4" className="text-center py-8 text-foreground/40">No transfers found.</td>
                </tr>
              ) : transfers.map(t => (
                <tr key={t.id} className="border-b border-foreground/5 hover:bg-white/[0.02] transition-colors">
                  <td className="py-3 px-4 text-foreground/70">{t.date}</td>
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <span className="text-foreground/80">{t.from_account?.name || 'Unknown'}</span>
                      <ArrowRightLeft size={14} className="text-foreground/30 shrink-0" />
                      <span className="text-foreground/80">{t.to_account?.name || 'Unknown'}</span>
                    </div>
                  </td>
                  <td className="py-3 px-4 text-right font-medium text-cyan-400">
                    ৳{t.amount.toLocaleString()}
                  </td>
                  <td className="py-3 px-4 text-foreground/50">{t.notes || '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
