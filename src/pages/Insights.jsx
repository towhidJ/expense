...
export default function Insights() {
  ...
  if (loading) return <div className="text-white/50 p-6">Analyzing your spending...</div>;

  return (
    <div className="space-y-6 animate-in">
      <div>
        <h1 className="text-2xl font-bold text-white">Spending Insights</h1>
        <p className="text-white/40 text-sm mt-1">What changed, where the money went, and an AI you can ask.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-6">
          <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-5">
            <h3 className="text-white font-semibold mb-3 flex items-center gap-2"><Sparkles size={16} className="text-violet-400" /> This Month's Findings</h3>
            <div className="space-y-2.5">
              ...
              {stats.findings.length === 0 && <p className="text-xs text-white/30">Not enough data yet — add some transactions first.</p>}
            </div>
          </div>

          <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-5">
            <h3 className="text-white font-semibold mb-3">Top Categories (this month)</h3>
            <div className="space-y-2.5">
                ...
                return (
                  <div key={cat}>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-white font-medium">{fmt(amt)} <span className="text-white/35 text-xs">({pct.toFixed(0)}%)</span></span>
                    </div>
              ...
              {stats.topCats.length === 0 && <p className="text-xs text-white/30">No expenses recorded this month.</p>}
            </div>
          </div>
        </div>

        <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-5 flex flex-col">
          <h3 className="text-white font-semibold mb-3 flex items-center gap-2"><Bot size={16} className="text-violet-400" /> Ask AI</h3>
          <div className="flex-1 space-y-3 overflow-y-auto min-h-48 max-h-96 mb-3">
            {chat.length === 0 && (
              <div className="space-y-2">
                <p className="text-xs text-white/35 mb-2">Try one of these:</p>
                {['Kon category te sobcheye beshi khoroch hocche?', 'How can I cut 10% of my spending?', 'Is my spending trend healthy?'].map(q => (
                  <button key={q} onClick={() => ask(q)} className="block w-full text-left text-xs bg-[#12122a] hover:bg-violet-500/10 text-white/50 hover:text-violet-300 rounded-xl px-3 py-2.5 transition-all">
                    💬 {q}
                  </button>
                ))}
              </div>
            ...
            {chat.map((m, i) => (
              <div key={i} className={`text-sm rounded-xl px-3.5 py-2.5 whitespace-pre-wrap ${m.role === 'user' ? 'bg-violet-500/15 text-violet-200 ml-8' : 'bg-[#12122a] text-white/70 mr-4'}`}>
                {m.text}
              </div>
            ))}
            {asking && <div className="text-sm bg-[#12122a] text-white/40 rounded-xl px-3.5 py-2.5 mr-4 animate-pulse">Thinking...</div>}
          </div>
          <form onSubmit={e => { e.preventDefault(); ask(); }} className="flex gap-2">
            <input
              type="text" value={question} onChange={e => setQuestion(e.target.value)}
              placeholder="Ask about your spending..."
              className="flex-1 min-w-0 bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-violet-500/50"
            />
          ...
          <p className="text-[10px] text-white/25 mt-2">Only aggregated totals are sent to the AI — not your individual transactions.</p>
        </div>
      </div>
    </div>
  );
