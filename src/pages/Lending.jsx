...
export default function Lending() {
  ...
  if (loading) return <div className="text-foreground/50 p-6">Loading dena-paona...</div>;

  return (
    <div className="space-y-6 animate-in">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Dena-Paona</h1>
          <p className="text-foreground/40 text-sm mt-1">Personal lending ledger — who owes you, whom you owe.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => setShowSettled(v => !v)}
            className={`px-3 py-2 rounded-xl text-sm font-medium transition-all border ${
              showSettled ? 'bg-foreground/10 text-foreground border-foreground/20' : 'bg-foreground/5 text-foreground/40 border-foreground/10 hover:bg-foreground/10 hover:text-foreground'
            }`}
          >
      ...
      {isAdding && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">New Loan Entry</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Person Name</label>
              <input
            ...
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Phone (Optional)</label>
              <input
            ...
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Return Date (Optional)</label>
              <input
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm text-foreground/60 mb-2">Money movement</label>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-foreground/10 rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-sm text-foreground/50 mb-6">
              {settlingLoan.type === 'loan_given' ? 'From' : 'To'}: <strong className="text-foreground">{settlingLoan.counterparty || settlingLoan.name}</strong>
              <span className="ml-2 text-foreground/40">(remaining {fmt(settlingLoan.remaining_balance)})</span>
            </p>
            <form onSubmit={handleSettle} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-6">
                <button type="button" onClick={() => setSettlingLoan(null)} className="px-5 py-2.5 rounded-xl text-foreground/60 hover:text-foreground hover:bg-foreground/5 transition-colors">Cancel</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {visiblePeople.map(person => {
          ...
          return (
            <div key={person.key} className="bg-card border border-foreground/10 rounded-2xl p-5 hover:border-foreground/20 transition-all">
                ...
                <div className="text-right">
                  {person.net !== 0 ? (
                    <>
                      ...
                      <p className="text-xs text-foreground/40">{person.net > 0 ? 'you will receive' : 'you owe'}</p>
                    </>
                  ) : (
              ...
              <div className="space-y-2">
                {shownLoans.map(loan => {
                  ...
                  return (
                    <div key={loan.id} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 border ${isPaid ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-muted border-foreground/5'}`}>
                      {isGiven
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-foreground/80">
                          ...
                          {!isPaid && Number(loan.remaining_balance) !== Number(loan.principal) && (
                            <span className="text-foreground/40"> · {fmt(loan.remaining_balance)} left</span>
                          )}
                        </p>
                ...
                {!showSettled && settledLoans.length > 0 && (
                  <p className="text-xs text-foreground/30 px-1">+ {settledLoans.length} settled loan{settledLoans.length > 1 ? 's' : ''} hidden</p>
                )}
              </div>
              ...
              {isExpanded && (
                <div className="mt-3 space-y-1.5 animate-in fade-in slide-in-from-top-2">
                  {person.repayments.map(rep => (
                    <div key={rep.id} className="flex items-center justify-between text-xs bg-muted rounded-lg px-3 py-2">
                      <span className="text-foreground/50">
                        {new Date(rep.date).toLocaleDateString()} · {rep.loan_type === 'loan_given' ? 'received' : 'paid'} via {rep.accounts?.name || 'account'}
                        {rep.notes && <span className="text-foreground/30"> · {rep.notes}</span>}
                      </span>
      ...
      {visiblePeople.length === 0 && !isAdding && (
        <div className="text-center py-12 border border-foreground/5 rounded-2xl bg-white/[0.02]">
          <HandCoins className="mx-auto text-foreground/20 mb-4" size={48} />
          <h3 className="text-foreground/60 font-medium">No dena-paona recorded</h3>
          <p className="text-foreground/40 text-sm mt-1">Track money you lent to or borrowed from people — with due dates and full history.</p>
        </div>
      )}
    </div>
  );
