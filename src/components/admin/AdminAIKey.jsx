  ...
  return (
    <div className="bg-foreground/5 border border-foreground/10 rounded-2xl p-6 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-semibold text-foreground">Gemini AI key</h2>
      </div>
      ...
      <div className="rounded-xl bg-foreground/5 border border-foreground/10 px-4 py-3 flex items-center gap-2 text-sm">
        {loading ? (
          <span className="text-foreground/40 flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Checking…</span>
        ) : status?.is_set ? (
          <span className="text-emerald-400 flex items-center gap-2">
            <Check className="w-4 h-4" /> Configured
            <span className="text-foreground/40 font-mono">{status.preview}</span>
            {status.updated_at && (
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <input
          ...
          className="flex-1 bg-foreground/5 border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-cyan-500/50 font-mono"
        />
        <button
          onClick={handleSave}
