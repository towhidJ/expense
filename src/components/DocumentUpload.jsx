...
export default function DocumentUpload({
  ...
  return (
    <div>
      <label className="block text-sm text-foreground/50 mb-1.5">{label}</label>

      {/* Already-saved attachments */}
      {existing.length > 0 && (
        <div className="space-y-2 mb-2">
          {existing.map((att) => (
            <div key={att.id} className="flex items-center gap-2 bg-foreground/5 border border-foreground/10 rounded-xl px-3 py-2">
              <FileText className="w-4 h-4 text-cyan-400 shrink-0" />
              <a
                href={att.file_url}
                target="_blank"
                rel="noreferrer"
                className="flex-1 min-w-0 text-sm text-foreground/80 truncate hover:text-cyan-400 flex items-center gap-1"
              >
                <span className="truncate">{att.file_name}</span>
                <ExternalLink className="w-3 h-3 shrink-0 opacity-60" />
              </a>
              {onRemoveExisting && (
                <button type="button" onClick={() => onRemoveExisting(att)} className="text-foreground/40 hover:text-red-400 shrink-0">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
        ...
        <div className="space-y-2 mb-2">
          {files.map((file, idx) => (
            <div key={idx} className="flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/20 rounded-xl px-3 py-2">
              ...
              <button type="button" onClick={() => removeFile(idx)} className="text-foreground/40 hover:text-red-400 shrink-0">
                <X className="w-4 h-4" />
              </button>
            </div>
          ))}
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 bg-foreground/5 border border-dashed border-foreground/15 rounded-xl px-4 py-2.5 text-foreground/50 text-sm hover:bg-foreground/10 hover:text-foreground/80 transition-all"
      >
