...
export default function AssetCard({ asset, totalExpense, onEdit, onDelete }) {
  ...
  return (
    <div className="p-5 rounded-2xl bg-foreground/5 backdrop-blur-xl border border-foreground/10 hover:border-foreground/15 transition-all group">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="text-right">
          <p className="text-sm font-bold text-foreground">৳{asset.value?.toLocaleString()}</p>
          <p className="text-xs text-foreground/30 mt-0.5">Value</p>
        </div>
      </div>

      <div className="bg-white/[0.02] rounded-xl p-3 border border-foreground/5 mb-4">
        <div className="flex justify-between items-center mb-1">
          <span className="text-xs text-foreground/50">Total Expenses Linked</span>
