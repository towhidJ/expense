...
export default function ShoppingListTab({
    ...
    return (
      <div className="px-5 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm ${it.is_bought ? 'text-foreground/40 line-through' : 'text-foreground'}`}>
            {it.name}{it.qty && <span className="text-foreground/40"> — {it.qty}</span>}
          </p>
        ...
        {canDelete && (
          <button onClick={() => act(() => deleteShoppingItem(it.id))}
            className="p-1.5 rounded-lg text-foreground/30 hover:text-red-400 hover:bg-foreground/5 shrink-0">
            <Trash2 size={14} />
          </button>
        )}
      </div>
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleAdd} className="bg-card border border-foreground/10 rounded-2xl p-4 flex flex-wrap gap-3">
        <input required type="text" value={form.name} placeholder="Item — e.g. Chal, Soyabean tel"
          onChange={e => setForm({ ...form, name: e.target.value })}
          className="flex-1 min-w-[180px] bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-cyan-500/50" />
        <input type="text" value={form.qty} placeholder="Qty — 2 kg"
          onChange={e => setForm({ ...form, qty: e.target.value })}
          className="w-32 bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-cyan-500/50" />
        <button type="submit"
      ...
      <div className="bg-card border border-foreground/10 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-foreground/10 flex items-center justify-between">
          <h3 className="text-foreground font-semibold">To Buy <span className="text-foreground/30 text-sm font-normal">({toBuy.length})</span></h3>
        </div>
        <div className="divide-y divide-foreground/5">
          {toBuy.map(it => <Row key={it.id} it={it} />)}
          {toBuy.length === 0 && (
            <p className="px-5 py-8 text-center text-foreground/40 text-sm">Nothing on the list. Add what the mess needs.</p>
          )}
        </div>
      </div>
      ...
      {bought.length > 0 && (
        <div className="bg-card border border-foreground/10 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-foreground/10 flex flex-wrap items-center justify-between gap-3">
            <h3 className="text-foreground font-semibold">Bought <span className="text-foreground/30 text-sm font-normal">({bought.length})</span></h3>
            {!convertOpen && (
          ...
          {convertOpen && (
            <form onSubmit={handleConvert} className="px-5 py-4 border-b border-foreground/10 bg-muted/50 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-foreground/60 text-sm">
                  All {bought.length} bought item{bought.length > 1 ? 's' : ''} become one itemized bazar expense.
                </p>
                <button type="button" onClick={() => setConvertOpen(false)} className="text-foreground/40 hover:text-foreground"><X size={16} /></button>
              </div>
              <div className="flex flex-wrap gap-3">
                <input required type="number" min="1" step="0.01" value={convertForm.amount} placeholder="Total amount (৳)"
                  ...
                  className="w-44 bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-emerald-500/50" />
                <input required type="date" value={convertForm.date}
                  onChange={e => setConvertForm({ ...convertForm, date: e.target.value })}
                  className="bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-emerald-500/50" />
                <input type="text" value={convertForm.note} placeholder="Note (optional)"
                  ...
                  className="flex-1 min-w-[160px] bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground text-sm focus:outline-none focus:border-emerald-500/50" />
