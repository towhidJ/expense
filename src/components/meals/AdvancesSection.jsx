...
export default function AdvancesSection({ advances, members, isManager, addAdvance, adjustAdvance, deleteAdvance }) {
  ...
  return (
    <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6 space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        {isManager && (
          <button
            ...
            className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white px-4 py-2 rounded-xl text-sm"
          >
      ...
      {isAdding && isManager && (
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-[#12122a] border border-white/10 rounded-xl p-4">
          <div>
            <label className="block text-sm text-white/60 mb-1">Member</label>
            <select required value={form.member_id} onChange={e => setForm({ ...form, member_id: e.target.value })} className="w-full bg-[#1a1a2e] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50">
          ...
          <div>
            ...
            <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="w-full bg-[#1a1a2e] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50">
          ...
          <div>
            ...
            <input required type="number" min="0.01" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} className="w-full bg-[#1a1a2e] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50" />
          </div>
          <div>
            <label className="block text-sm text-white/60 mb-1">Date</label>
            <input required type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} className="w-full bg-[#1a1a2e] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50" />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm text-white/60 mb-1">Note</label>
            <input type="text" value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} className="w-full bg-[#1a1a2e] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50" placeholder="optional" />
          </div>
          <div className="sm:col-span-2 flex justify-end gap-3">
            <button type="button" onClick={() => setIsAdding(false)} className="px-5 py-2 rounded-xl text-white/60 hover:text-white hover:bg-white/5">Cancel</button>
      ...
      <div className="flex flex-wrap gap-2">
        {membersWithBalance.map(m => (
          <span key={m.id} className={`px-3 py-1.5 rounded-lg border text-xs ${m.adv > 0 ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-white/10 bg-white/5 text-white/40'}`}>
            {m.display_name}{m.status !== 'approved' ? ` (${m.status})` : ''}: {fmt(m.adv)}
          </span>
        ))}
        {membersWithBalance.length === 0 && <span className="text-white/40 text-sm">No members yet.</span>}
      </div>
      ...
      {advances.length > 0 && (
        <div className="divide-y divide-white/5 border-t border-white/10 pt-2">
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white text-sm truncate">{memberName(a.member_id)} <span className="text-white/40">· {meta.label}</span></p>
                  <p className="text-white/40 text-xs">{a.date}{a.note ? ` · ${a.note}` : ''}</p>
                </div>
                ...
                {isManager && (
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-[#12122a] border border-white/10 text-white/60 hover:text-red-400"
                  >
                    <Trash2 size={13} />
                  </button>
                )}
          ...
          {advances.length > 6 && (
            <button onClick={() => setShowAll(v => !v)} className="w-full text-center text-xs text-white/40 hover:text-white py-2">
