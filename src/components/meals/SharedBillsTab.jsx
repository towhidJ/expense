    ...
    <div className="space-y-6">
      ...
      {isManager && open && (
        <form onSubmit={handleSave} className="bg-card border border-foreground/10 rounded-2xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-foreground font-semibold">New Shared Bill</h3>
            <button type="button" onClick={() => setOpen(false)} className="text-foreground/40 hover:text-foreground"><X size={18} /></button>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Total amount (৳)</label>
              <input required type="number" min="1" step="0.01" value={form.amount}
                onChange={e => setForm({ ...form, amount: e.target.value })}
                className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Split</label>
              <select value={form.split_type}
                onChange={e => setForm({ ...form, split_type: e.target.value })}
                className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50 appearance-none cursor-pointer">
                <option value="equal" className="bg-muted">Equally</option>
                <option value="custom" className="bg-muted">Custom amounts</option>
              </select>
            </div>
            <div>
              ...
              {approved.map(m => (
                form.split_type === 'equal' ? (
                  <label key={m.id} className={`flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm border cursor-pointer ${
                    form.included[m.id] ? 'bg-cyan-500/10 border-cyan-500/30 text-white' : 'bg-muted border-foreground/10 text-white/40'
                  }`}>
                  ...
                  <div key={m.id} className="flex items-center gap-2 bg-muted border border-foreground/10 rounded-xl px-3 py-1.5">
                    <span className="text-foreground/60 text-sm flex-1 truncate">{m.display_name}</span>
                    <input type="number" min="0" step="0.01" value={form.custom[m.id] || ''}
                      placeholder="0"
                      onChange={e => setForm({ ...form, custom: { ...form.custom, [m.id]: e.target.value } })}
                      className="w-24 bg-transparent border border-foreground/10 rounded-lg px-2 py-1.5 text-foreground text-sm text-right focus:outline-none focus:border-cyan-500/50" />
                  </div>
                )
              ))}
            </div>
      ...
      <div className="space-y-4">
        {sharedExpenses.map(bill => {
          ...
          return (
            <div key={bill.id} className="bg-card border border-foreground/10 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-foreground/10 flex flex-wrap items-center gap-3">
                ...
                <div className="flex-1 min-w-[180px]">
                  <p className="text-foreground font-medium">{bill.title}</p>
                ...
                <div className="text-right">
                  <p className="text-foreground font-semibold">{fmt(bill.amount)}</p>
                ...
                {isManager && (
                  <button onClick={() => { if (confirm('Delete this bill and its shares?')) act(() => deleteSharedExpense(bill.id)); }}
                    className="p-2 rounded-lg text-foreground/30 hover:text-red-400 hover:bg-foreground/5">
                    <Trash2 size={15} />
                  </button>
                )}
              </div>
                ...
                {shares.map(s => {
                  ...
                  return (
                    <div key={s.id} className="px-5 py-2.5 flex items-center gap-3">
                      ...
                      <span className="text-foreground/60 text-sm">{fmt(s.share_amount)}</span>
                      {isManager ? (
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input type="checkbox" checked={s.paid}
                            onChange={e => act(() => toggleSharePaid(s.id, e.target.checked))}
                            className="accent-emerald-500 w-4 h-4" />
                          <span className={`text-xs ${s.paid ? 'text-emerald-400' : 'text-foreground/30'}`}>paid</span>
                        </label>
                      ) : (
