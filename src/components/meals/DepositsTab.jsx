    ...
    <div className="space-y-4">
      ...
      {(isAdding || editing) && (
        <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-white mb-4">{editing ? 'Edit Deposit' : 'New Deposit'}</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-white/60 mb-1">Member</label>
              <select required value={form.member_id} onChange={e => setForm({ ...form, member_id: e.target.value })} className="w-full bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50">
            ...
            <div>
              ...
              <input required type="number" min="0.01" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} className="w-full bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={form.note} onChange={e => setForm({ ...form, note: e.target.value })} className="w-full bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500/50" placeholder="optional" />
            </div>
        ...
        {deposits.map(d => (
          <div key={d.id} className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-medium truncate">{memberName(d.member_id)}</p>
              <p className="text-white/40 text-xs">{d.date}{d.note ? ` · ${d.note}` : ''}</p>
            </div>
            <span className="text-emerald-400 font-semibold">{fmt(d.amount)}</span>
            {isManager && (
              <div className="flex gap-1 ml-2">
                <button onClick={() => { setEditing(d); setIsAdding(false); setForm({ member_id: d.member_id, amount: d.amount, date: d.date, note: d.note || '' }); }} className="p-1.5 rounded-lg bg-[#12122a] border border-white/10 text-white/60 hover:text-cyan-400">
                  <Edit2 size={14} />
                </button>
                <button onClick={() => { if (confirm('Delete this deposit?')) deleteDeposit(d.id).catch(err => alert('Cannot delete: ' + err.message)); }} className="p-1.5 rounded-lg bg-[#12122a] border border-white/10 text-white/60 hover:text-red-400">
                  <Trash2 size={14} />
                </button>
              </div>
            )}
      ...
      {!isManager && <p className="text-white/30 text-xs">Only the manager records deposits (they receive the cash).</p>}
    </div>
  );
}
