  ...
  return (
    <div className="inline-flex items-center gap-1">
      <button
        ...
        className="w-7 h-7 flex items-center justify-center rounded-lg bg-muted border border-foreground/10 text-foreground/60 hover:text-foreground disabled:opacity-30"
      >
        <Minus size={13} />
      </button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <button onClick={() => shiftDate(-1)} className="p-2 rounded-xl bg-card border border-foreground/10 text-foreground/60 hover:text-foreground">
          <ChevronLeft size={18} />
        </button>
        <input
          type="date"
          ...
          className="bg-card border border-foreground/10 rounded-xl px-4 py-2 text-foreground focus:outline-none focus:border-cyan-500/50"
        />
        <button onClick={() => shiftDate(1)} className="p-2 rounded-xl bg-card border border-foreground/10 text-foreground/60 hover:text-foreground">
          <ChevronRight size={18} />
        </button>
        <span className="text-foreground/40 text-sm">{dayName}</span>
        {isManager && (
          <button
            onClick={() => setHolidayModal({ title: holiday?.title || 'Meal Holiday', menu: holiday?.menu || '' })}
            className={`ml-auto flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs ${holiday ? 'border-pink-500/40 bg-pink-500/10 text-pink-300' : 'border-foreground/10 bg-card text-white/50 hover:text-white'}`}
          >
      ...
      {holiday && (
        <div className="bg-pink-500/10 border border-pink-500/20 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            {holiday.menu && <p className="text-foreground/60 text-sm mt-0.5">Special food / nasta: {holiday.menu}</p>}
            <p className="text-foreground/30 text-xs mt-1">Meal holiday — regular meals are usually off; record the feast cost as a "Feast / Special" expense.</p>
          </div>
        </div>
      )}
      ...
      {holidayModal && isManager && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setHolidayModal(null)}>
          <form onSubmit={saveHoliday} className="bg-card border border-foreground/10 rounded-2xl p-6 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h3 className="text-foreground font-semibold flex items-center gap-2"><PartyPopper size={16} className="text-pink-400" /> Holiday — {date}</h3>
              <button type="button" onClick={() => setHolidayModal(null)} className="text-foreground/40 hover:text-foreground"><X size={18} /></button>
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Title</label>
              <input required type="text" value={holidayModal.title} onChange={e => setHolidayModal({ ...holidayModal, title: e.target.value })} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50" placeholder="e.g. Eid Day, Friday Feast" />
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Special food / nasta plan (optional)</label>
              <textarea rows={2} value={holidayModal.menu} onChange={e => setHolidayModal({ ...holidayModal, menu: e.target.value })} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-pink-500/50" placeholder="e.g. Biriyani + borhani, morning khichuri" />
            </div>
            <div className="flex justify-between gap-3">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setHolidayModal(null)} className="px-4 py-2 rounded-xl text-foreground/60 hover:text-foreground hover:bg-foreground/5 text-sm">Cancel</button>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
                  ...
                  <Fragment key={member.id}>
                    <tr className="border-b border-foreground/5">
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs ${guestTotal > 0 ? 'border-purple-500/40 text-purple-300 bg-purple-500/10' : 'border-foreground/10 text-white/50 bg-muted'}`}
                        >
                    ...
                    {guestOpen[member.id] && (
                      <tr className="border-b border-foreground/5 bg-white/[0.02]">
                        <td className="px-4 py-2 text-foreground/40 text-xs">Guest meals</td>
                        {SLOTS.map(s => (
              ...
              {approvedMembers.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-foreground/40">No approved members yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {!isManager && (
        <p className="text-foreground/30 text-xs">You can only record your own meals. The manager can edit everyone's.</p>
      )}
    </div>
  );
}
