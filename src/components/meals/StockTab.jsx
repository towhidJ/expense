    ...
    <div className="space-y-4">
      ...
      {isAdding && (
        <div className="bg-card border border-foreground/10 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-foreground mb-4">New Stock Item</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Name</label>
              <input required type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Rice / চাল" className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm text-foreground/60 mb-1">Quantity</label>
                <input type="number" min="0" step="0.01" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
              </div>
              <div>
                <label className="block text-sm text-foreground/60 mb-1">Unit</label>
                <input type="text" value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })} placeholder="kg" className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
              </div>
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Low-stock alert below</label>
              <input type="number" min="0" step="0.01" value={form.low_stock_threshold} onChange={e => setForm({ ...form, low_stock_threshold: e.target.value })} placeholder="optional" className="w-full bg-muted border border-foreground/10 rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-cyan-500/50" />
            </div>
            <div>
              <label className="block text-sm text-foreground/60 mb-1">Expiry date</label>
      ...
      <div className="bg-card border border-foreground/10 rounded-2xl divide-y divide-foreground/5">
          ...
          <div key={item.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="w-20 bg-muted border border-foreground/10 rounded-lg px-2 py-1.5 text-foreground text-sm focus:outline-none focus:border-cyan-500/50"
              />
            ...
            {isManager && (
              <button onClick={() => { if (confirm(`Remove "${item.name}" from stock?`)) deleteStockItem(item.id).catch(err => alert(err.message)); }} className="p-1.5 rounded-lg bg-muted border border-foreground/10 text-foreground/60 hover:text-red-400">
                <Trash2 size={14} />
              </button>
            )}
          </div>
        ...
        {stockItems.length === 0 && (
          <div className="px-4 py-10 text-center text-foreground/40 text-sm">No stock items yet. Add what's in the pantry.</div>
        )}
      </div>
      <p className="text-foreground/30 text-xs">Anyone can adjust quantities (stock in/out) — it's a shared list.{isManager ? ' Only a manager can remove an item.' : ''}</p>
    </div>
  );
}
