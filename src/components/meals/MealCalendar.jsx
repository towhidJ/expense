          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 text-white/40 text-left">
                <th className="px-3 py-3 font-medium sticky left-0 bg-[#1a1a2e]">Date</th>
                {cols.map(m => (
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.day} className={`border-b border-white/5 last:border-0 ${
                  r.holiday ? 'bg-amber-500/5' : r.date === todayStr ? 'bg-cyan-500/5' : ''
                }`}>
                  <td className="px-3 py-2 whitespace-nowrap sticky left-0 bg-[#1a1a2e]">
                    <span className={r.date === todayStr ? 'text-cyan-400 font-semibold' : 'text-white/80'}>
                      {pad(r.day)}
                    </span>
                    <span className="text-white/30 text-xs ml-1.5">{r.weekday}</span>
                    {r.holiday && (
                  ...
                  <td className="px-3 py-2 text-right text-white/60">{r.dayTotal || ''}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-white/10 text-white/80 font-semibold">
                <td className="px-3 py-3 sticky left-0 bg-[#1a1a2e]">Total</td>
                {cols.map(m => (
