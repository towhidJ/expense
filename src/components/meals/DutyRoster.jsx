      ...
      {isManager && (
        <div className="bg-[#1a1a2e] border border-white/10 rounded-2xl p-6">
          ...
          <div className="space-y-2 mb-4">
            {dutyTypes.map(type => (
              <div key={type.id}>
                <div className="flex items-center gap-3 bg-[#12122a] border border-white/10 rounded-xl px-4 py-2.5">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs border ${rotationPanel === type.id ? 'bg-cyan-500/20 border-cyan-500/40 text-cyan-400' : 'bg-[#1a1a2e] border-white/10 text-white/50 hover:text-white'}`}
                  >
                ...
                {rotationPanel === type.id && (
                  <div className="mt-2 bg-[#12122a] border border-white/10 rounded-xl p-4 space-y-3">
                    <p className="text-white/40 text-xs">Pick who's in the rotation and their order, then generate assignments.</p>
                    <div className="space-y-1.5">
                      {approvedMembers.map(m => {
                        ...
                        return (
                          <div key={m.id} className="flex items-center gap-2 bg-[#1a1a2e] border border-white/10 rounded-lg px-3 py-1.5">
                            <input type="checkbox" checked={inRotation} onChange={() => toggleInRotation(type.id, m.id)} className="accent-cyan-500" />
                            <span className={`flex-1 text-sm ${inRotation ? 'text-white' : 'text-white/40'}`}>{m.display_name}</span>
                            {inRotation && (
                              <>
                                <span className="text-white/30 text-xs">#{idx + 1}</span>
                                <button disabled={idx === 0} onClick={() => moveInRotation(type.id, idx, -1)} className="p-1 text-white/40 hover:text-white disabled:opacity-20"><ArrowUp size={13} /></button>
                                <button disabled={idx === order.length - 1} onClick={() => moveInRotation(type.id, idx, 1)} className="p-1 text-white/40 hover:text-white disabled:opacity-20"><ArrowDown size={13} /></button>
                              </>
                            )}
                          </div>
                        );
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <input type="number" min="1" max="62" value={genDays} onChange={e => setGenDays(e.target.value)} className="w-20 bg-[#1a1a2e] border border-white/10 rounded-lg px-2 py-1.5 text-white text-sm" />
                      <span className="text-white/40 text-xs">days from this week</span>
                      <button
