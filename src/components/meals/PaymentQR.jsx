    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {open && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setOpen(null)}>
          <div className="bg-card border border-foreground/10 rounded-2xl p-6 w-full max-w-xs text-center" onClick={e => e.stopPropagation()}>
            <button onClick={() => setOpen(null)} className="ml-auto block text-foreground/40 hover:text-foreground mb-2"><X size={18} /></button>
            <h3 className="text-foreground font-semibold mb-1">{open === 'bkash' ? 'bKash' : 'Nagad'} Send Money</h3>
            <p className="text-foreground/40 text-xs mb-4">{open === 'bkash' ? bkash : nagad}</p>
            <div className="bg-white rounded-xl p-4 inline-block">
              <QRCodeSVG value={payload(open === 'bkash' ? 'bKash' : 'Nagad', open === 'bkash' ? bkash : nagad, amount, groupName)} size={200} />
            </div>
            <p className="text-foreground/30 text-xs mt-3">Scan and send manually — this isn't an automatic payment link.</p>
          </div>
        </div>
      )}
    </div>
